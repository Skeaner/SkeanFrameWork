package me.skean.skeanframework.widget.indeterminateviews;
...
public class IndeterminateCheckBox extends AppCompatCheckBox implements IndeterminateCheckable {
    ...
    public IndeterminateCheckBox(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.indeterminateCheckBoxStyle);
    }

    public IndeterminateCheckBox(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.IndeterminateCheckBox, defStyleAttr, 0);
        try {
            ...
            if (d != null) {
                ...
                if (iconId == defaultIcon) {
                    if (Build.VERSION.SDK_INT >= 23) {
                        setButtonDrawable(defaultIcon);
                    } else {
                        setButtonDrawable(IndeterminateUtils.tintDrawable(this, defaultIcon));
                    }
                } else setButtonDrawable(d);
            }
            // Read the XML attributes
            final boolean indeterminate = a.getBoolean(R.styleable.IndeterminateCheckBox_indeterminate, false);
            if (indeterminate) {
